...
function FormPanel({ form, loading, set, setAddr, setTax, setCont, onClose, onSubmit }) {
  return (
    <div
      ...
      >
        ...
        <div style={{ padding: "20px 24px 28px", display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <Field label="Registration Name">
                  <Input value={form.tax_scheme.registration_name} placeholder="Bridgtow District Council"
                    onChange={e => setTax("registration_name", e.target.value)} />
                </Field>
                <Field label="Company ID">
                  <Input value={form.tax_scheme.company_id} placeholder="12356478"
                    onChange={e => setTax("company_id", e.target.value)} />
                </Field>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                <Field label="Exemption Reason">
                  <Input value={form.tax_scheme.exemption_reason} placeholder="Local Authority"
                    onChange={e => setTax("exemption_reason", e.target.value)} />
                </Field>
                <Field label="Scheme ID">
                  <Input value={form.tax_scheme.scheme_id} placeholder="UK VAT"
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Name">
                <Input value={form.contact.name} placeholder="Mr Fred Churchill"
                  onChange={e => setCont("name", e.target.value)} />
              </Field>
              <Field label="Email">
                <Input type="email" value={form.contact.email} placeholder="fred@iytcorporation.gov.uk"
                  onChange={e => setCont("email", e.target.value)} />
              </Field>
              <Field label="Telephone">
                <Input value={form.contact.telephone} placeholder="0127 2653214"
                  onChange={e => setCont("telephone", e.target.value)} />
              </Field>
              <Field label="Telefax">
                <Input value={form.contact.telefax} placeholder="0127 2653215"
          ...
          {/* Footer */}
          <div style={{ paddingTop: 4, display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button
              onClick={onClose}
              style={{
